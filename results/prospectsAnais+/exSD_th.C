...
void exSD_th(int minInt=10) // bines de 0.1 keV 1 keV -> 10 bines
{
  ...
  error = exclusion->Initialize((char*)"exclusion.dat");
  if (error<0) return;
...
gPad->Update();
  ...
  std::vector<double> bkg = {1, 0.1};
  std::vector<double> exp = {365, 3650};
  std::vector<double> th = {0.1, 0.15, 0.25};
  int n = bkg.size();
  for (int ii=0; ii<n; ii++)
  {
    exclusion->SetExposure(exp[0]); // in kgxday
    exclusion->SetLinearBkg(bkg[ii]); // in kgxday
    exclusion->SetThreshold(th[1]); // in keV
