TGraph * grshadeNaC;
TGraph * grshadeNaI;
void compRateAnaisDAMA(double si=1e-4, double mass=10) // pbarn y GeV
{
...
int i;
  ...
  TFile * f = new TFile ("QFmeasurementsANAIS_keVee.root","read");
  if (!f) {std::cout << " CANNOT READ QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  
  TFile * fother = new TFile ("$DMANALYSIS/templates/quenchingNaIee.root","read");
  if (!fother) {std::cout << " CANNOT READ QUENCHING FACTOR FILE!!" << std::endl; exit(0);}

  TGraph * gQNaC=(TGraph*)f->Get("gQNaANAISee");
  if (!gQNaC) {std::cout << " CANNOT FIND gQNaC IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  TGraph * gQNaCl=(TGraph*)f->Get("gQNaANAISee_l"); // low error
  if (!gQNaCl) {std::cout << " CANNOT FIND gQNaCl IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  TGraph * gQNaCh=(TGraph*)f->Get("gQNaANAISee_h"); // high errr
  if (!gQNaCh) {std::cout << " CANNOT FIND gQNaCh IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  TGraph * gQIC=(TGraph*)f->Get("gQIANAISee");
  if (!gQIC) {std::cout << " CANNOT FIND gQIC IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  TGraph * gQICl=(TGraph*)f->Get("gQIANAISee_l");
  if (!gQIC) {std::cout << " CANNOT FIND gQICl IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  TGraph * gQICh=(TGraph*)f->Get("gQIANAISee_h");
  if (!gQICh) {std::cout << " CANNOT FIND gQICh IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}

   // TRETYAK
  TGraph * gQNaT=(TGraph*)fother->Get("gQNaTretyakee");
  if (!gQNaT) {std::cout << " CANNOT FIND TGRAPH IN QUENCHING FACTOR FILE!!" << std::endl; exit(0);}
  TGraph * gQIT=(TGraph*)fother->Get("gQITretyakee");
...
//htotC->Draw("lhist");
//htotT->Draw("lhistsame");
hIC->GetYaxis()->SetTitle("Rate (c/keV/kg/day)");
hIC->GetXaxis()->SetTitle("energy (keV)");
hIC->Draw("lhist");
hICl->Draw("lhistsame");
hICh->Draw("lhistsame");
hNaC->Draw("lhistsame");
hNaCl->Draw("lhistsame");
hNaCh->Draw("lhistsame");
//hNaT->Draw("lhistsame");
//hIT->Draw("lhistsame");
hNaS->Draw("lhistsame");
hIS->Draw("lhistsame");
...
TGraph * grshadeNaC = new TGraph(2*nn);
for (i=0;i<nn;i++)
{
...
nn=hICh->GetNbinsX();
TGraph * grshadeIC = new TGraph(2*nn);
for (i=0;i<nn;i++)
{
...
leg->AddEntry(haux,"Na [ANAIS-112]");
...
leg->AddEntry(haux2,"I [ANAIS-112]");
...
leg->Draw();
 ...
 TPaveText * pp = new TPaveText(0.7,0.25,0.90,.5,"NDC");
...
//htotC->Draw("lhist");
//htotT->Draw("lhistsame");
hICm->GetYaxis()->SetTitle("Rate (c/keV/kg/day)");
hICm->GetXaxis()->SetTitle("energy (keV)");
hICm->Draw("lhist");
hIClm->Draw("lhistsame");
hIChm->Draw("lhistsame");
hNaCm->Draw("lhistsame");
hNaClm->Draw("lhistsame");
hNaChm->Draw("lhistsame");
//hNaTm->Draw("lhistsame");
//hITm->Draw("lhistsame");
hNaSm->Draw("lhistsame");
hISm->Draw("lhistsame");
hICm->GetXaxis()->SetRangeUser(0,6);
hICm->GetYaxis()->SetRangeUser(-0.05,0.20);

// Shade for errors
nn=hNaChm->GetNbinsX();
TGraph * grshadeNaCm = new TGraph(2*nn);
for (i=0;i<nn;i++)
{
...
nn=hIChm->GetNbinsX();
TGraph * grshadeICm = new TGraph(2*nn);
for (i=0;i<nn;i++)
{
...
leg = new TLegend(0.65,0.7,0.99,0.99);
leg->AddEntry(hNaS,"Na Q=0.3");
haux = (TH1F*)hNaCm->Clone();
haux->SetFillStyle(3013);
haux->SetFillColor(kRed);
//leg->AddEntry(hNaC,"Na [Collar]");
...
leg->Draw();
 leg->SetFillColor(kWhite);
  ...
  pp = new TPaveText(0.7,0.55,0.90,.7,"NDC");
