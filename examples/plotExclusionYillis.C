...
//  uses pseudo Yillis method to optain an optimum interval, 
//  with minimum interval minInt (in bins)
...
{
  int minInt = 10; // 10 bins minimum interval in exclusion calculation
  ...
  for (int i=0; i<4; i++)
  {
    ...
    gSI = plothl.GetSIExclusion(1,1e3,minInt); // from 1 GeV to 1 TeV
  ...
  {
    exclusion->SetLinearBkg(bkg[i]); // in c/keVee/kg/d
    gSD = plothl.GetSDpExclusion(1,1e3,minInt); // from 1 GeV to 1 TeV
